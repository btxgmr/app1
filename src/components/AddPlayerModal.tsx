    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="w-full max-w-sm p-5 rounded-3xl bg-[#0e131f] border border-white/[0.12] shadow-2xl space-y-4">
        <div className="flex justify-between items-center pb-2 border-b border-white/[0.08]">
          <h3 className="text-base font-black text-white">Dodaj Nowego Zawodnika</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5 text-xs">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="font-bold text-neutral-400 mb-1 block">Punkty początkowe ({unit})</label>
              <input
                type="number"
                value={score}
          ...
          <div>
            <label className="font-bold text-neutral-400 mb-1 block">Wybierz Emoji</label>
          ...
          <button
            type="submit"
            className="w-full py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm flex items-center justify-center space-x-1.5 shadow-lg transition-all"
          >
            <Plus className="w-4 h-4" />
            <span>Dodaj do tabeli</span>
          </button>
        </form>
      </div>
    </div>
